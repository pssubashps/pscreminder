...
if ( ! function_exists('preset_checkbox'))
{
    function preset_checkbox($field = '', $value = '', $preset_value = '')
    {
        if(is_array($preset_value)) {
            if(count($preset_value) > 0) {
                foreach($preset_value as $preval) {
                    if ($value == $preval)
                    {
                        return set_checkbox($field, $preval, TRUE);
                    }
                }
...
if ( ! function_exists('preset_value'))
{
    function preset_value($field = '', $value = '', $preset_value = '')
    {
        if ($value == $preset_value)
        {
            return set_value($field, $preset_value, TRUE);
        }
        else
        {
